...
papers.to_csv('neo4j_import/papers.csv', index=False)
...
for idx, row in df.iterrows():
    ...
    corresponding = ast.literal_eval(row['correspondingAuthor'])
    ...
    for i in range(len(author_ids)):
        ...
        author_paper_relations.append({
            'paperDOI': paper_doi,
            'authorID': author_ids[i],
            'correspondingAuthor': corresponding[i] if i < len(corresponding) else False
        })
    
# Create dataframes and remove duplicates
authors_df = pd.DataFrame(authors_list).drop_duplicates(subset=['authorID'])
authors_df.to_csv('neo4j_import/authors.csv', index=False)

author_paper_df = pd.DataFrame(author_paper_relations)
author_paper_df.to_csv('neo4j_import/author_paper_relations.csv', index=False)
...
conferences.to_csv('neo4j_import/conferences.csv', index=False)
...
conf_paper_relations.to_csv('neo4j_import/conference_paper_relations.csv', index=False)
...
journals.to_csv('neo4j_import/journals.csv', index=False)
...
journal_paper_relations.to_csv('neo4j_import/journal_paper_relations.csv', index=False)
...
keywords_df.to_csv('neo4j_import/keywords.csv', index=False)

keyword_paper_df = pd.DataFrame(keyword_paper_relations)
keyword_paper_df.to_csv('neo4j_import/keyword_paper_relations.csv', index=False)

# 6. Extract Citations (Paper to Paper relationships)
citations_list = []
for idx, row in df.iterrows():
    paper_doi = row['paperDOI']
    if isinstance(row['citedPapers'], str):
        cited_papers = ast.literal_eval(row['citedPapers'])
        
        for cited_paper in cited_papers:
            citations_list.append({'paperDOI': paper_doi, 'citedPaperDOI': cited_paper})
   
citations_df = pd.DataFrame(citations_list)
citations_df.to_csv('neo4j_import/citations.csv', index=False)

print("Files saved in the 'neo4j_import' directory.")

# some papers dont have abstract
# some journals have missing volumes
# year, edition is float instead of int